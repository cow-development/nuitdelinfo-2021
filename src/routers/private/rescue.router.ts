...
export class RescueRouter implements IMonitored {
  ...
  private _setupRoutes() {
    ...
    this._router.put('/confirm/:rescueId', (req: Request, res: Response, next: NextFunction) => {
      this._rescueController.confirm(req as SignedRequest, res, next)
        .then(result => {
          res.status(200).json(result)
        })
        .catch(error => next(error));
    ...
    this._router.put('/update/:rescueId', (req: Request, res: Response, next: NextFunction) => {
      this._rescueController.update(req as SignedRequest, res, next)
        .then(result => {
          res.status(200).json(result)
        })
        .catch(error => next(error));
